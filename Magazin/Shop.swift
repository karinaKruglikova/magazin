...
//        }
//    }
    func dayte(tovar:String, count:Int) -> [String] {
        let nyjniyeTovar = items.filter({ (item: String) -> Bool in
            return item == tovar
        })
        if nyjniyeTovar.count >= count {
            var tovaryOtdat:[String] = []
            for aTovar in nyjniyeTovar {
                tovaryOtdat.append(aTovar)
                if tovaryOtdat.count == count {
                    return tovaryOtdat
                }
            }
        }
